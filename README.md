...
### Смена layout при навигации

При клиентской навигации сервер отдаёт только содержимое тега `<page>`, поэтому layout целевой страницы применить нельзя — в браузере остаётся layout текущей страницы.

Чтобы этого не происходило, layout страницы записывается в состояние навигации. Если у страницы, на которую идёт навигация, layout отличается от текущего, сервер отвечает заголовком `Page-Reload`, и клиент выполняет полную загрузку страницы.

```
@page
@model AltLayoutModel
@{
    Layout = "_LayoutAlt";
}
...
Layout сравнивается по пути файла, а не по имени: страницы из разных папок могут ссылаться на разные файлы по одному и тому же относительному имени.

Так же полная загрузка выполняется при смене area и при смене версии сайта.
...
```
OpenGraph.Set("locale", "ru_RU");
OpenGraph.Set("video", "https://example.com/promo.mp4");
```
